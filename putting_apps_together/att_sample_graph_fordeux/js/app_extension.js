...
var beforeRender = function(){
		 ...
		 setTimeout(function(){
				if(app.getContainer){
						ac = app.getContainer();
					if(ac && ac.ownerCt){
								header = ac.ownerCt.header;
								if(header && header.addTool){
										
										// insert to the left of the close button
										// (which may be hidden but will be far on right)
										
										index = header.items.findIndexBy(function(item){
										...
										header.add(index, {
												...
												handler: function(event, toolEl, panel){
													vToExcel = FusionCharts(jQuery("span[id^=embed-chart-]",app.$el).attr("id")).getDataAsCSV();
													vToExcel = vToExcel.split(",").join("\t")
													vToExcel = vToExcel.split('\r').join('\u2028')
														prompt("Copy this text and paste to excel spreadsheet.",vToExcel)
												}
													
										});
										header.add(index, {
												type: 'print',
												tooltip: 'Save graph as image.',
												handler: function(event, toolEl, panel){
													FusionCharts(jQuery("span[id^=embed-chart-]",app.$el).attr("id")).exportChart();
												}
...
interval = setInterval(function(){
		if(Presto.BasicWebApp){
				Presto.BasicWebApp.prototype.beforeRender = beforeRender;	
				clearInterval(interval);
		}
